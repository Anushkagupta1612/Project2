...
# This blockchain is decentralized to 3 nodes named: Anushka, Ashu and someone
...
def mine_block():
    ...
    #transactions
    blockchain.add_transaction(sender = node_address , receiver = 'Anushka', amount = 1)
...
def replace_chain():
    ...
    if is_chain_replaced:
        response = {'message': 'The nodes had different chains so they had to be replaced by the longest chain',
                    'new_chain' : blockchain.chain}
    else:
        response = {'message': 'Anushka, no changes were made.',
                    'new_chain' : blockchain.chain}
    return jsonify(response), 200
